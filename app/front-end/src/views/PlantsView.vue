...
onMounted(async () => {
  let response = await fetch('http://localhost:8001/api/plants');
  let genus = await fetch('http://localhost:8001/api/genus');
...
watch(filter, async () => {
  let response = await fetch('http://localhost:8001/api/plants?page=1' + [...Object.entries(filter)].map(([key, value]) => `&${key}=${value}`).join(''));
  response = await response.json();
  plants.value = await response.data;
  totalPages.value = response.totalPages;
  currentPage.value = 1;
...
watch(currentPage, async () => {
  let response = await fetch('http://localhost:8001/api/plants?page=' + currentPage.value + [...Object.entries(filter)].map(([key, value]) => `&${key}=${value}`).join(''));
  response = await response.json();
  plants.value = await response.data;
  totalPages.value = response.totalPages;
});
